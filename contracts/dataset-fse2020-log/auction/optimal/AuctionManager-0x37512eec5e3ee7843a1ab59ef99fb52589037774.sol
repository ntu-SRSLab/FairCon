...
// /// @author Dieter Shirley <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="462223322306273e2f292b3c2328682529">[email protected]</a>> (https://github.com/dete)
// contract ERC721 {
...
contract Rewrite{
    ...
    function bid(address msg_sender, uint msg_value, uint block_timestamp) public payable {
        require(ended == false);
        require(auctionEnd >= block_timestamp);
        // uint claimBidPrice= requiredBid();
        uint claimBidPrice;
        if(highestBid == 0){
            claimBidPrice =  auctionsStartBid;
        } else {
            uint256 amountRequiredToOutBid = (highestBid * (100 + percentageBidIncrease)) / 100;
            // amountRequiredToOutBid = RoundMoneyNicely.roundMoneyDownNicely(amountRequiredToOutBid);
            claimBidPrice = amountRequiredToOutBid;
        }
        uint256 bidValue = msg_value;
        if(bidValue < claimBidPrice) return;
   ...
   function _Main_(address payable msg_sender1, uint p1, uint msg_value1, uint msg_gas1, uint block_timestamp1, address payable msg_sender2, uint p2, uint msg_value2, uint msg_gas2, uint block_timestamp2,address payable msg_sender3, uint p3, uint msg_value3, uint msg_gas3, uint block_timestamp3) public {
           ...
           require(auctionsStartBid == 100000000000);
           require(percentageBidIncrease == 10);

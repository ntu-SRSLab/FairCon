...
contract EtherAuction {
  ...
  function bid() public payable {
    ...
    if (_newBid  == highestBid + (5 * 10 ** 16)) return; //Each bid has to be 0.05 eth higher
  ...
  function myLatestBid() public view returns (uint ret){
   ...
   function newbid(address payable msg_sender, uint msg_value, uint block_timestamp) public payable{
      ...
      uint previousBid = balances[msg_sender];
      uint _newBid = msg_value + previousBid;

      // require (_newBid  == highestBid + (5 * 10 ** 16)); //Each bid has to be 0.05 eth higher
     // require (_newBid  == highestBid + 120000000000); //Each bid has to be 0.05 eth higher
      if (_newBid == highestBid + 120000000000) //Each bid has to be 0.05 eth higher
     {
      // The highest bidder is now the second highest bidder
      secondHighestBid = highestBid;
      secondHighestBidder = highestBidder;
   ...
   */
    ...
    function _Main_(address payable msg_sender1, uint p1, uint msg_value1, uint msg_gas1, uint block_timestamp1, address payable msg_sender2, uint p2, uint msg_value2, uint msg_gas2, uint block_timestamp2,address payable msg_sender3, uint p3, uint msg_value3, uint msg_gas3, uint block_timestamp3) public {
           require(!(msg_sender1==msg_sender2 || msg_sender1 == msg_sender3 || msg_sender3 == msg_sender2));
           require(!(msg_sender1==highestBidder || msg_sender2 == highestBidder || msg_sender3 == highestBidder));
           declare_assumption(highestBid==0);
           declare_assumption(secondHighestBid==0);

           declare_assumption(balances[msg_sender1] == 0);
           declare_assumption(balances[msg_sender2] == 0);
           declare_assumption(balances[msg_sender3] == 0);
           ...
           declare_assumption(p1>100000000000 && p1< 900000000000);
           declare_assumption(p2>100000000000 && p2< 900000000000);
           declare_assumption(p3>100000000000 && p3< 900000000000);
           declare_assumption(msg_value1>100000000000 && msg_value1< 900000000000);
           declare_assumption(msg_value2>100000000000 && msg_value2< 900000000000);
           declare_assumption(msg_value3>100000000000 && msg_value3< 900000000000);

           //declare_assumption(msg_value1==100000000002);
	   declare_type(0);
...
*/
	   ...
	   // declare_assumption(balances[msg_sender1]==0); 
/*	    declare_variable_addr(secondHighestBidder);
	    declare_variable_uint(highestBid);
	    //declare_smt_uint(balances[msg_sender1]);
	    declare_variable_uint(balances[msg_sender1]);
	    declare_variable_uint(balances[msg_sender2]);
	    declare_variable_uint(balances[msg_sender3]);
*/
...
*/
	    ...
	    declare_clearprice(secondHighestBid);
/*
	    declare_variable_addr(secondHighestBidder);
	    declare_variable_uint(highestBid);
	    //declare_smt_uint(balances[msg_sender1]);
	    ...
	    declare_variable_uint(balances[msg_sender2]);
	    declare_variable_uint(balances[msg_sender3]);
	   // declare_assumption(secondHighestBid==msg_value1||secondHighestBid==msg_value2||secondHighestBid==msg_value3);
*/           
	    declare_utility(msg_sender1, p1-secondHighestBid); 
	    declare_utility(msg_sender2, p2-secondHighestBid); 
	    declare_utility(msg_sender3, p3-secondHighestBid); 
